...
function generateUniqueId() {
  let newTime = Date.now().toString(36);
...
function addTodo() {
  ...
  while (texts.trim() === "") {
    texts = prompt("Enter your new to-do: ");

    if (texts.trim() === "") {
      console.log("To-do cannot be empty. Please submit a valid to-do.");
      return;
    }
  }
...
function markTodoCompleted() {
  if (todos.length === 0) {
    console.log("There is no todo to complete.");
    return;
  }
  ...
  const markTodo = parseInt(
    prompt("enter the number to-do that marks as completed: ")
  );
  // 3. Validasi input: Pastikan nomor adalah angka, dalam rentang yang valid (1 sampai jumlah to-do)
  if (isNaN(markTodo) || markTodo < 1 || markTodo > todos.length) {
  ...
  let mark = markTodo - 1;
  const todo = todos[mark];

  if (todo.isCompleted) {
    console.log(`To-do "${todo.text}" is already marked as completed.`);
  } else {
    todo.isCompleted = true;
    console.log(`To-do "${todo.text}" completed`);
  }
...
function deleteTodo() {
  ...
  const markTodo = parseInt(
    prompt("enter the number to-do that marks as deleted: ")
  );
  ...
  const deleteT = markTodo - 1;
  const deleteTodo = todos.splice(deleteT, 1)[0];
  // 5. Beri feedback ke user bahwa to-do berhasil dihapus
  console.log(`To-do "${deleteTodo.text}" deleted`);
}
